...
const MyDayCard = ({ habits, checkins, toggleCheckin }) => {
  ...
  const getPriorityColor = (priority) => {
    switch (priority) {
      case 'alta': return '#dc2626'; // vermelho
      case 'media': return '#ea580c'; // laranja
      case 'baixa': return '#16a34a'; // verde
      case 'baixissima': return '#2563eb'; // azul
      default: return '#16a34a'; // verde como padrão
    }
  };
  ...
  const getHabitsForDate = (date) => {
    ...
    return weekHabits.filter(habit => {
      if (habit.type === 'daily') {
        ...
        return habit.customDays && habit.customDays.includes(dayIndex);
      }
      return false;
    });
  };
      ...
      <div className="p-8">
        ...
        <div className="space-y-6">
          {periodNames.map((period, periodIndex) => {
            ...
            return (
              <div key={period} className="bg-white/30 rounded-xl p-6 backdrop-blur-sm">
                <div className="flex flex-col items-center mb-4">
                  <div className="mb-2">
                    {periodEmojis[periodIndex]}
                  </div>
                  <h3 className="text-lg font-medium calm-text-primary text-center" style={{ fontFamily: 'Asimovian, sans-serif' }}>
                    {period}
                  </h3>
                </div>
                ...
                <div className="space-y-3">
                  {periodHabits.map(habit => {
                    ...
                    return (
                      <div 
                        key={habit.id} 
                        className="flex items-center justify-center p-3 rounded-lg cursor-pointer hover:bg-white/20 transition-all duration-200"
                      ...
                      >
                        <span 
                          ...
                          style={{ 
                            ...
                            color: isChecked ? undefined : getPriorityColor(habit.priority || 'baixa')
                          }}
                        >
                          {habit.name}
                        </span>
                  ...
                  {periodHabits.length === 0 && (
                    <p 
                      className="calm-text-muted text-center py-6"
                      style={{ fontFamily: 'Dosis, sans-serif' }}
                    >
                      Nenhum hábito para {period.toLowerCase()}
                    </p>
                  )}
                </div>
              </div>
